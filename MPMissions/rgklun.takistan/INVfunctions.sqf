...
INV_Heal = {
		...
		player groupChat format[localize "STRS_inv_items_medkit_benutzung"];
		uiSleep 5;
		player setdamage 0;
		[player] call medical_system_heal;
		player groupChat format[localize "STRS_inv_items_medkit_fertig"];
		true
};
...
INV_RemoveIllegalStorage = {
	...
	_owner = _object getVariable "vehicle_owner";
	if !(isNil "_owner") then {
		_owner = [_owner] call unit_by_name;
		if !(isNull _owner) then {
			[_owner, "(illegal items - vehicle)", _drugs_value] call player_update_warrants;
		};
	};
	

	[_object, _arrayname, "drug"] call INV_StorageRemoveKindOf;
	(format ["if (player == %2) then {player groupChat ""%1 had drugs in its trunk, you removed them.""}; titletext [format ['%1 had $%3 worth of illegal items!'], ""PLAIN DOWN""];", _vcl, player, [_drugs_value,","] call format_integer]) call broadcast;
	true
};
...
stats_parse_inventory = {
	private ["_inventory","_array","_item","_amount","_name"];
	_inventory = [player] call player_get_inventory;
	_return = [];
	
	for "_i" from 0 to (count _inventory - 1) do {
		_array = _inventory select _i;
		_item = _array select 0;
		_amount = [(_array select 1)] call decode_number;
		_name = _item call INV_GetItemName;
		
		
		if (_amount > 0) then {
			_return set [count _return,_array];
		};
...
INV_GetItemLicense3 = { private["_license_3"]; _license_3 = (((_this call INV_GetItemArray) select 4) select 3); if (isNil "_license_3") then {diag_log format ["DEBUG:%1 is missing license_3",_this];};  if (isNil "_license_3") exitWith {""}; _license_3};
INV_GetItemLicense4 = { private["_license_4"]; _license_4 = (((_this call INV_GetItemArray) select 4) select 4); if (isNil "_license_4") then {diag_log format ["DEBUG:%1 is missing license_4",_this];}; if (isNil "_license_4") exitWith {""}; _license_4};
