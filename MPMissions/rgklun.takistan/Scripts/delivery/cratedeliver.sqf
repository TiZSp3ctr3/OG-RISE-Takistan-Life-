	...
	if (_num <= 30) then {_class = "Misc_cargo_cont_net2"; _payment = 300000};
	
	if ((_num > 30) && (_num < 80)) then {_class = "Fort_Crate_wood"; _payment = 150000};
	
	if (_num > 80 ) then {_class = "Misc_cargo_cont_net1"; _payment = 250000};
	
	_crate = _class createVehicle (getMarkerPos "crate_spawn_1");
	
	format[' _crate addaction ["Check Crate Contents","Scripts\delivery\crate_notify.sqf",false,1,false,true,"",""];'] call broadcast;
	
	_crate setpos (getMarkerPos "crate_spawn_1");

	crate_extension = vehicleVarName player;

	_cratename = ["crate_%1", crate_extension];
