...
INV_CreateWeapon = {
	...
	if (_in_hands) then {
		...
		_message = format ["%1 bought a(n) %2 at %3",name player,_class,_crate];
		[_message, "Weapon Purchase"] call mp_log;
	};
};
...
INV_CreateMag = {
	...
	_message = format ["%1 bought %4 %2 at %3",name player,_class,_crate,_amount];
	[_message, "Mag Purchase"] call mp_log;
