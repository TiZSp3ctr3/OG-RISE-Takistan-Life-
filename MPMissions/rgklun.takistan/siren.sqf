...
if (_art == "activate") then
{
	...
	if (_turnOn) then
	{
		format ["[0,0,0,[""client"", %1,%2]] execVM ""siren.sqf"";", player,isesu] call broadcast;
...
if (_art == "client") then
{
	...
	_light1 = "#lightpoint" createVehicle (getpos _vcl);
	...
	while {( ((time < _starttime+_playtime) or (_driver == driver _vcl)) and (alive _vcl) and (call compile format["%1_Sirene_on", _vcl]))} do {
		if ((player distance _vcl < 500) and (!player_dead)) then
		{
				_driver = driver _vcl;
				...
				if !(_esu) then {
					if (speed _vcl > 60) then {
						...
						uiSleep siren_longDur;
					} else {
						_vcl say ["Siren_Short", 1];
						uiSleep siren_shortDur;
					};
				} else {
					_vcl say ["ambulance", 1];
					uiSleep esusiren;
				};
