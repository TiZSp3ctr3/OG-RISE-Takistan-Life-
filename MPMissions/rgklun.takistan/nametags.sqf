...
name_tags_draw = {
	...
	if ([_target] call player_human &&  _distance < 25) exitWith {
		...
		if (_skin in viceskins and ([_target] call player_cop or [_target] call player_opfor)) exitWith {};
...
name_3d_tags_draw = {
...

	{
		...
		if (true) then {
			...
			if (not(alive _vehicle) || not(isPlayer _target) ||
			...
			if ((_aimin_at_center || _target == cursorTarget) && _distance > 25) then {
				...
				if !(([_target,"restrained"] call player_get_bool) and ([_target,"ziptied"] call player_get_bool)) then {
					if ([_target] call player_insurgent) then {
...
name_tags_loop = {
	...
	if (alive player) then {
		...
		if ([player] call player_civilian) exitWith {};

		{
			if ([player] call player_cop and [_x] call player_opfor and !jointops) then {name_tags_side_units = [];};
			if ([player] call player_opfor and [_x] call player_cop and !jointops) then {name_tags_side_units = [];};
		} foreach name_tags_side_units;
			...
			if !(isNil "_x" and isNull _x) then {
				if (typeName _x == "OBJECT") then {
					if ((alive _x) and (isPlayer _x) and not(_x in name_tags_side_units)) then {
						if (jointops) then {
								if (([player] call player_cop) and ([_x] call player_cop or [_x] call player_opfor)) then {name_tags_side_units set [count(name_tags_side_units), _x];};
								if (([player] call player_opfor) and ([_x] call player_opfor or [_x] call player_cop)) then {name_tags_side_units set [count(name_tags_side_units), _x];};
								if (([player] call player_insurgent) and ([_x] call player_insurgent)) then {name_tags_side_units set [count(name_tags_side_units), _x];};
								if (([player] call player_esu) and ([_x] call player_esu)) then {name_tags_side_units set [count(name_tags_side_units), _x];};
							} else {
								if (([player] call player_cop) and ([_x] call player_cop)) then {name_tags_side_units set [count(name_tags_side_units), _x];};
								if (([player] call player_opfor) and ([_x] call player_opfor)) then {name_tags_side_units set [count(name_tags_side_units), _x];};
								if (([player] call player_insurgent) and ([_x] call player_insurgent)) then {name_tags_side_units set [count(name_tags_side_units), _x];};
								if (([player] call player_esu) and ([_x] call player_esu)) then {name_tags_side_units set [count(name_tags_side_units), _x];};
						};
					};
