...
diag_log "Taki Life Init - My Stats Functions Loading";
...
stats_get_faction = {
	private ["_player"];
	_player = _this select 0;
	if ([_player] call player_opfor) exitWith {"Opfor"};
	if ([_player] call player_cop) exitWith {"Cop"};
	if ([_player] call player_civilian) exitWith {"Civilian"};
	if ([_player] call player_esu) exitWith {"ESU"};
	if ([_player] call player_insurgent) exitWith {"Insurgent"};
	""
};
...
diag_log "Taki Life Init - My Stats Functions Loaded";
