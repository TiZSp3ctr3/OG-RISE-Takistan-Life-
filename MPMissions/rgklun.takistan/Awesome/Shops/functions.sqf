...
rg_shops_fnc_GetVehiclePictureByClassName = {
	private["_vehicle_class", "_result", "_cfg"];
	_vehicle_class = _this select 0;
	_result = "";

	_cfg = (configFile >> "CfgVehicles" >> _vehicle_class);

    _result = if (isText(_cfg >> "picture")) then {
...
rg_shops_fnc_GetWeaponPictureByClassName = {
	private["_weapon_class", "_result", "_cfg"];
	_weapon_class = _this select 0;
	_result = "";

	_cfg = (configFile >> "cfgWeapons" >> _weapon_class);
...
rg_shops_fnc_GetMagazinePictureByClassName = {
	private["_magazine_class", "_result", "_cfg"];
	_magazine_class = _this select 0;
	_result = "";

	_cfg = (configFile >> "cfgMagazines" >> _magazine_class);
...
shop_setup = {
	...
	while { _i < _count } do {
	/*
		player groupChat format ["Count Size: %1",_count];
		player groupChat format ["HOLD LOOP: %1",_i];
		*/
		...
		player groupChat format ["Count Size: %1",_count];
		player groupChat format ["HOLD LOOP: %1",_i];
		player groupChat format ["SHOP ID: %1 : %2",_shop_id,_shop_object];
		*/
	...
	/*
	_i = 0;
	_count = count INV_ItemShops;

	player groupChat format ["Count Size AFTER LOOP: %1",_count];

	while { _i < _count } do {
		player groupChat format [" OPENING SHOP: %1",_i];

		[_i] call shop_build_buy_list;
		[_i] call shop_build_sell_list;
		[_i] call shop_open;
...
shop_build_buy_list  =  {
	...
	while { _i < _count } do {
		...
		if (isNil "_license_3" or isNil "_license_4") then {
			diag_log format ["DEBUG: Masterarray entry %1 has error zero divisor",_item];
		};
...
shop_update_sell_list =  {
	...
	while { _i < _count && not(isNull(findDisplay shopDialog_idd))} do {
		private["_index", "_data", "_valid", "_item", "_infos", "_type", "_item_classname", "_picture"];
		...
		if (_valid) then {
			...
			_item_classname = _infos call INV_GetItemClassName;
			_picture = "";
			...
			switch _type do {
				...
				case "Vehicle": {
					_picture = [_item_classname] call rg_shops_fnc_GetVehiclePictureByClassName;
				};
				case "Weapon": {
					_picture = [_item_classname] call rg_shops_fnc_GetWeaponPictureByClassName;
				};
				case "Magazine": {
					_picture = [_item_classname] call rg_shops_fnc_GetMagazinePictureByClassName;
				};
				case "backpack": {
					_picture = "";
				};
			...
			if (_picture != "") then {
				lbSetPicture [sellItem_list_idc, _index, _picture];
			};
...
shop_update_buy_list =  {
	...
	_count = (count _items);
	while { _i < _count } do {
		private["_index", "_data", "_item", "_infos", "_type", "_item_classname", "_picture"];
		_data = _items select _i;
		...
		switch _type do {
			case "Item": {
			_picture = [_item_classname] call get_item_image;
			};
			case "Vehicle": {
				_picture = [_item_classname] call rg_shops_fnc_GetVehiclePictureByClassName;
			};
		...
		if (_picture != "") then {
			lbSetPicture [buyItem_list_idc, _index, _picture];
		};
...
shop_buy_item_validate_data = {
	...
	if (_needsLicense && (isciv or isesu) && !(_license_1 call INV_HasLicense)) exitWith {
	...
	if (_total_price > _player_money) exitWith {
		["You do not have enough shekels to buy this item", _quiet]  call shop_set_status_message; nil
	};
	...
	if (_putInHands && _hasPrimaryWeapon && _isWeapon && _isLauncher) exitWith {
		["You cannot do that with a Primary weapon", _quiet] call shop_set_status_message; nil
	};	
	
	if (_putInHands && _hasSecondaryWeapon && _isWeapon && _isRifle) exitWith {
		["You cannot do that with a Secondary weapon", _quiet] call shop_set_status_message; nil
	};
...
shop_sell_item = {
	...
	_isDrug = (_kind == "drug" or _kind == "organ");
	...
	_message = format ["%1 sold %2 %3 for $%4",name player,_amount, _item_name, strM(_total_return)];
	[_message, "Item Sale"] call mp_log;
};
...
shop_sell_vehicle = {
	...
	_vehicles = [_class, 100] call shop_get_vehicles_by_class;
	...
	_message = format ["%1 sold a %2 for $%3",name player,_item_name, strM(_total_return)];
	[_message, "Vehicle Sale"] call mp_log;
...
shop_sell_gear = {
	...
	["backpack"] call RG_fnc_iSave;
	["weapons"] call RG_fnc_iSave;
	["magazines"] call RG_fnc_iSave;
	["inventory"] call RG_fnc_iSave;

	player groupChat format["You sold %1 %2 for $%3", _amount, _item_name, strM(_total_return)];
	
	_message = format ["%1 sold %2 %3 for $%4",name player,_amount, _item_name, strM(_total_return)];
	[_message, "Weapon Sale"] call mp_log;
};
...
shop_buy_item = {
	...
	_message = format ["%1 bought a(n) %2 at %3",name player,_item_name];
	[_message, "Item Purchase"] call mp_log;
...
shop_distribute_drug_sale = {
	...
	format['if((name player) in %1) then {player groupchat "You received $%2 from a drug sale"; [player, %2] call transaction_dynamiccuntflap;};', _gang_members, strM(_income)] call broadcast;
};
...
shop_drug_search = {
...

	{
		if (true) then {
			...
			player groupChat format["This civilian bought $%1 worth of illegal items. from %2-%3!", strM(_profit), _player, (name _player)];

			[_player, "(drug-trafficking)", _profit] call player_update_warrants;
			private["_message"];
			_message = format["%1-%2 is wanted for trafficking $%3 worth of illegal items.!", _player, (name _player), strM(_profit)];
...
shop_buy_vehicle = {
	_this spawn {
		...
		_log = format ["%1 bought a %2 at a vehicle shop!", (name vehicle player), _item_name];

		[_log, "Vehicle Purchase"] call mp_log;
...
shop_open_dialog = {
	...
	if (not([_shop_id] call shop_is_open)) exitWith {
		player groupChat format["This shop is not open yet.Opening them now"];
...
shop_set_fuel_consumed = {
	...
	fuel_consumed = _fuel_consumed;
	publicVariable "fuel_consumed";
};
