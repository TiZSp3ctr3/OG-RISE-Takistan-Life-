...
factory_globals = {
	...
	all_factories =
	[
		[ Vehiclefactory, "factory1", "Vehicle Factory", dummyobj, vfacspawn, _vehiclefactory, 3000000, "Fabrikablage1", "vehiclequeue"],
		...
		//[ tairshop, "factory6", "Terrorist Vehicle Factory", dummyobj,tairspawn, _tairfactory, 1000000, "Fabrikablage6", "tavehiclequeue"],
...
factory_init = {
	...
	[_workers_name, 40] call stats_init_variable;
...
factory_item_enqueue = {
	...
	if (_factory_money < _total_due) exitWith {
		factory_enqueue_active = false;
		player groupChat format["%1-%2, you do not have enough shekels in your %3 storage to produce %4 %5", _player, (name _player), _factory_name, _amount, _item_name];
	};
...
factory_item_create = {
	...
	_factory_spawn_pos = [_factory_spawn] call FNC_getpos;
	_factory_crate = _factory select factory_crate;

	if(count (nearestobjects [[_factory_spawn] call FNC_getpos ,["Ship","car","motorcycle","truck","Air"], 5]) > 0) exitwith{ player groupchat "there is a vehicle within 5 meters of the spawn!"};
...
factory_validate_enqueue_item = {
	...
	if (_production_cost > _factory_money) then {
		_message2 = "There is not enough shekels in the factory storage to produce this item";
...
factory_hire_workers = {
	...
	_total_cost = (_workers_count * facworkercost);

	if (_money < _total_cost) exitWith {
		player groupChat format["%1-%2, you do not have enough shekels in your inventory to hire %3 workers for your %4", _player, (name _player), _workers_count, _factory_name];
	};
...
factory_buy = {
	...
	if (_money < _factory_cost) exitWith {
		player groupChat format["%1-%2, you do not have enough shekels in your inventory to buy this factory", _player, (name _player)];
	};
