...
time_loop = {
	...
	while {true} do {
		...
		_i = _i + 1;
		if (time_loop_exit) exitWith {};
		if (_i > 480) exitWith { [false] spawn time_loop; };
	};
};
...
time_move = {
	_this spawn {
		...
		// Sync the initialize all the globals for all clients
		[[], "RG_time_init_globals", []] call mp_aware_me;
	};
};
...
time_reset = {
	_this spawn {
		...
		[[], "RG_time_exitloop", []] call mp_aware_me;
		uiSleep 3;
