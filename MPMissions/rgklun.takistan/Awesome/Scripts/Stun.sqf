...
if (_action == 1) then {
	...
	if (((currentWeapon _unit) == "M9") || ((currentWeapon _unit) == "M9SD") || ((currentWeapon _unit) == "glock17_EP1") || ((currentWeapon _unit) == "UZI_EP1") || ((currentWeapon _unit) == "UZI_SD_EP1")) then {
		format["
...
if (_action == 2) then {
	...
	private["_is_stungun"];
	_is_stungun = ((currentWeapon _shooter) == ("M1014") or (currentWeapon _shooter) == ("AA12_PMC") or (currentWeapon _shooter) == ("MP5A5") );
	
	if ( _is_stungun && _distance <= 50 ) then {
		_pass spawn stun_shot_close;
	} else { if (_is_stungun && (_distance > 50) && (_distance <= 100)) then {
...
if (_action == 3) then {
	...
	_men = nearestobjects [getPosAtl _unit, ["CAManBase"], 3] - [_unit];
	_man = _men select 0;
	
	// bailflag
	if ((_man in shopusearray) || (_man == bailflag)) exitWith {};
	if (_man getVariable "downed") exitWith {};
	
	if ([_man] call check_safezone) exitWith {};
	if (typeOf _unit in ["USMC_LHD_Crew_Red"]) exitWith {};
