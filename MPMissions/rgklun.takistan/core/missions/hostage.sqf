	...
	while {true} do {
		for [{_i=0}, {_i < (count workplacejob_hostage_serverarray)}, {_i=_i+1}] do {
			if (isNull ((workplacejob_hostage_serverarray select _i) select 0)) then {
				...
				uiSleep ((workplacejob_hostage_break)*60);
...
//	[format["%1 (%2, %3, %4) - hostage: group created %5",  round(time), player, (name player), (getPlayerUID player), _group]] call l4a;
	...
	while {true} do {
		...
		if(hostage1 distance getmarkerpos "hostagezone" > 150) exitwith {
			"
			server globalchat ""The Hostage taker has fled the area! Cops get $50000"";
			_copplayernumber = playersNumber west;
			_copbonus = 50000;
			if (iscop) then {[player, _copbonus] call transaction_dynamiccuntflap; player sidechat format[""you received $%1 for hostage taker fleeing the area"", _copbonus];};
			" call broadcast;
			uiSleep 2;
		...
		if(player distance hostage1 > 150) exitWith {
			...
			_copplayernumber = playersNumber west;
			_copbonus = 50000;
			if (iscop) then {[player, _copbonus] call transaction_dynamiccuntflap; player sidechat format[""you received $%1 for the successful rescue of the hostage"", _copbonus];};
			" call broadcast;
			uiSleep 2;
